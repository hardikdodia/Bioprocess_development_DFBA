...
for i = 1:N_MEAS_REL
    if nnz((cumsum(ODist.*0.370.*diff([0 timesFBA]).*fluxr(:,i)'))>0)>0 
        ...
        imin = min(find((cumsum(ODist.*0.370.*diff([0 timesFBA]).*fluxr(:,i)'))>0));
        if ~isempty(imin)
...
for i=1:N_REAC
    ...
    if ismember(i,ind_fluxm) 
        [~,count] = ismember(i,ind_fluxm) ;   
        LB((0:Nm-1)*size(Aeqs,2)+i,1) = min(fluxm_lb(:,count),xmax3((0:Nm-1)*size(Aeqs,2)+i,1));
        UB((0:Nm-1)*size(Aeqs,2)+i,1) = max(fluxm_lb(:,count),xmax3((0:Nm-1)*size(Aeqs,2)+i,1));
    end
