...
end_time = input("Enter the ending time of Experiments in hours : \n");
...
Expt_Culture_Conc_CAbs_1 = FindAbsConc(Expt_Data_Culture_CAbs_1{:,2:end},Std_Curve_Data1{:,2:end}); % mM
Expt_NCC_Conc_CAbs_1 = FindAbsConc(Expt_Data_NCC_CAbs_1{:,2:end},Std_Curve_Data2{:,2:end}); % mM

Expt_Culture_Conc_CAbs_2 = FindAbsConc(Expt_Data_Culture_CAbs_2{:,2:end},Std_Curve_Data1{:,2:end}); % mM
Expt_NCC_Conc_CAbs_2 = FindAbsConc(Expt_Data_NCC_CAbs_2{:,2:end},Std_Curve_Data2{:,2:end}); % mM
...
Expt_Culture_Time_TAbs(end) = end_time;
...
% change = input("Should the Mf of Tryptophan be multiplied by 2?(1/0) :  \n");
change =1;
...
if Mf_Data_CAbs_Available == 1
    Mf_CAbs = Mf_Data_CAbs{1,2:(end-misc)};
    for i = 1:(size(Expt_NCC_Conc_CAbs,2)-misc)
        Kd_CAbs(i) = lsqcurvefit(@(p,t) FindKd(p, t, M0_CAbs(i), V0, F, Mf_CAbs(i), Feed_Time), MfKd0(2), Expt_NCC_Time_CAbs, Expt_NCC_Conc_CAbs(:,i), Kd_LB, Kd_UB); % h-1
    end
elseif Mf_Data_CAbs_Available == 0 
    for i = 1:(size(Expt_NCC_Conc_CAbs,2)-misc)
...
% relative ncc but abs mf

if misc>0

    for i = 1:misc
        MfKd_misc(:,i) = lsqcurvefit(@(p,t) FindMfKd(p, t, M0_Rel(i), V0, F, Feed_Time), MfKd0, Expt_NCC_Time_CAbs, Expt_NCC_Conc_CAbs(:,(end-misc+i)), [Mf_LB Kd_LB], [Mf_UB Kd_UB]); % h-1
        Mf_misc(i)= MfKd_misc(1,i);
        Kd_misc(i)= MfKd_misc(2,i);
    end
...
if misc >0
    Mf_CAbs = [Mf_CAbs table2array(Mf_Data_CAbs(1,((end-misc+1):end)))];
    Kd_CAbs = [Kd_CAbs Kd_misc];
end

MfKd_Data_CAbs = array2table(["Mf" Mf_CAbs; "Kd" Kd_CAbs],"VariableNames",["MfKd" Names_CAbs]);
writetable(MfKd_Data_CAbs,excelfilename,"Sheet","MfKd CAbs Results");

M0_CAbs = [M0_CAbs Expt_Culture_Conc_CAbs(1,((end-misc+1):end))];
...
MARS_Tend =  Expt_Culture_Time_TAbs(end);

disp("Performing MARS Interpolation for Experimental Culture Data...");

[MARS_OD,MARS_OD_LB,MARS_OD_UB] = MARS(MARS_dt,Expt_Culture_Time_TAbs,0,MARS_Tend,Expt_Data_Culture_TAbs_1.OD,Expt_Data_Culture_TAbs_2.OD,"OD",2); % OD600
[MARS_eYFP,MARS_eYFP_LB,MARS_eYFP_UB] = MARS(MARS_dt,Expt_Culture_Time_TAbs,12,MARS_Tend,Expt_Data_Culture_TAbs_1.eYFP,Expt_Data_Culture_TAbs_2.eYFP,"eYFP",2); % g/L
[MARS_TAbs,MARS_TAbs_LB,MARS_TAbs_UB] = MARS(MARS_dt,Expt_Culture_Time_TAbs,0,MARS_Tend,Expt_Data_Culture_TAbs_1{:,4:end},Expt_Data_Culture_TAbs_2{:,4:end},Names_TAbs(3:end),2); % g/L
[MARS_CAbs,MARS_CAbs_LB,MARS_CAbs_UB] = MARS(MARS_dt,Expt_Culture_Time_CAbs,0,MARS_Tend,Expt_Culture_Conc_CAbs_1,Expt_Culture_Conc_CAbs_2,Names_CAbs,2); %mM
[MARS_Rel,MARS_Rel_LB,MARS_Rel_UB] = MARS(MARS_dt,Expt_Culture_Time_Rel,0,MARS_Tend,Expt_Data_Culture_Rel_1{:,2:end},Expt_Data_Culture_Rel_2{:,2:end},Names_Rel,2); 

MARS_Data_Time = [0:MARS_dt:MARS_Tend].';
